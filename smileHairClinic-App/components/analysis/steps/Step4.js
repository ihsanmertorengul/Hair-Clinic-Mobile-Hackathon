...
export default function Step4({ navigation, route }) {
    ...
    const BACKEND_URL = "http://192.168.1.108:5001/analyze";
    ...
    const updateAnalysisStep4 = async (photoUrl) => {
        try {
            if (!analysisId) {
    ...
    const uploadToCloudinary = async (uri) => {
        try {
            ...
            data.append('file', {
                uri: uri,
                type: 'image/jpeg',
                name: 'image4.jpg', // ismi sabit veya değişken yapabilirsiniz
            });
    ...
    useEffect(() => {
        const playSound = async () => {
            const { sound } = await Audio.Sound.createAsync(
                require('../../../assets/sesler/step4/giris.mp3')
            );
    ...
    useEffect(() => {
        ...
        const sub = Accelerometer.addListener(({ x, y, z }) => {
            ...
            if (
                roundedPitch >= -20 &&
                roundedPitch <= 20 &&
                roundedRoll >= -20 &&     // ✔ ROLL kontrolü eklendi
                roundedRoll <= 20 &&      // ✔ ROLL kontrolü eklendi
                !autoShot &&
                !analysisOk
            ) {
    ...
    const playSuccessSound = async () => {
        const { sound } = await Audio.Sound.createAsync(
            require('../../../assets/sesler/step4/true.mp3')
        );
        await sound.playAsync();
        sound.setOnPlaybackStatusUpdate((status) => {
            if (status.didJustFinish) {
    ...
    const playFailedSound = async () => {
        const { sound } = await Audio.Sound.createAsync(
            require('../../../assets/sesler/step4/false.mp3')
        );
        await sound.playAsync();
        sound.setOnPlaybackStatusUpdate((status) => {
            if (status.didJustFinish) {
    ...
    const playAnalysisSound = async () => {
        const { sound } = await Audio.Sound.createAsync(
            require('../../../assets/sesler/step4/analiz.mp3')
        );
        await sound.playAsync();
        sound.setOnPlaybackStatusUpdate((status) => {
            if (status.didJustFinish) {
    ...
    const takePhotoAndSend = async () => {
        ...
        // 🔊 Analiz başlıyor sesi (hemen çal)
        playAnalysisSound(); // Burada ses başlatılıyor
        ...
        try {
            ...
            console.log("AI Analiz Sonucu:", json);

            const vertexOk = json.is_vertex === true;
            ...
            if (vertexOk) {
                ...
                if (photo.uri) {
                    ...
                    await updateAnalysisStep4(cloudinaryLink);
                }
            } else {
                playFailedSound();
            }
    ...
    const renderResultCard = () => {
        ...
        return (
            <View style={styles.resultCard}>
                ...
                <Text style={[styles.resultStatus, result.vertex_ok ? styles.statusSuccess : styles.statusError]}>
                    {result.vertex_ok ? 'Tepe fotoğrafı uygun' : 'Uygun değil'}
                </Text>
            </View>
        );
    };
        ...
        <SafeAreaView style={styles.safeArea}>
            <View style={styles.container}>
                ...
                <View style={styles.infoCard}>
                    <Text style={styles.infoBadge}>Adım 4 / 5</Text>
                    <Text style={styles.infoTitle}>Tepe (Vertex) Çekimi</Text>
                    <Text style={styles.infoSubtitle}>
                        Telefonu başınızın üzerine getirip tepe bölgesini merkezleyin. Cihazı olabildiğince dik tutun.
                    </Text>
                ...
                <View style={styles.cameraShell}>
                    <View
                        style={[
                            ...
                            { borderColor: result?.vertex_ok ? '#12B76A' : '#E4E7EC' },
                        ]}
                    >
                        <CameraView
                            style={styles.camera}
                ...
                {result?.vertex_ok && (
                    <TouchableOpacity
                        ...
                        onPress={() => navigation.navigate('Step5', { analysisId })}
                    >
